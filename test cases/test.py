#!/usr/bin/env python
# currently only works for systolic and diastolic. In progress!!
"""
...
class OmronBP786(object):
    ...
    def acquire(self, image_file='tc3.jpg'):        
        self.image = imutils.resize(cv2.imread(image_file), height=500)
        gray = cv2.cvtColor(self.image, cv2.COLOR_BGR2GRAY)
##        blurred = cv2.GaussianBlur(gray, (5, 5), 0)# kernel width and height should be odd
        ...
        for c in cnts:
            ...
            if float(h) / w > 1.1 and float(h) / w < 9 and h > 60:# only grab large countours of certain ratio
